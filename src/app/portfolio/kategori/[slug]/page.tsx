 ...
 *
 * The old suffix ("Lihat N proyek yang sudah dikerjakan SAVOY")
...
function categoryDescription(category: ProjectCategory, count: number): string {
  return `${category.description} ${count} proyek nyata dari arsip SAVOY.`;
}
...
export default async function CategoryPage(props: SlugPageProps) {
  const { slug } = await props.params;
  const category = categoryBySlug(slug);
