...
const SEO_DESCRIPTION_SUFFIXES = [
  ...
  "Dirancang, diproduksi, dan dipasang langsung oleh tim SAVOY.",
...
function buildAlt(project: ManifestProject, index: number): string {
  ...
  const parts = [`${resolved.name} custom karya SAVOY`];
  const place = buildPlace(project);
  if (place) parts.push(`di ${place}`);
  ...
  beforeImage && afterImage
    ? {
        before: {
          ...
          alt: "Kondisi ruangan sebelum pengerjaan custom furniture SAVOY",
        ...
        after: {
          ...
          alt: "Ruangan yang sama setelah pemasangan custom furniture SAVOY",
