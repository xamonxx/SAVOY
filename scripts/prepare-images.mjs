...
const PROCESS_FRAMES = [
  {
    ...
    slug: "konsultasi",
    match: ["KONSULTASI"],
    alt: "Konsultan SAVOY menjelaskan sampel material HPL kepada pasangan klien di studio",
  },
  {
    index: "02",
    slug: "survey-lokasi",
    match: ["SURVEY-LOKASI", "SURVEY LOKASI"],
    alt: "Tim teknis SAVOY mengukur dinding dengan laser meter dan mencatat posisi stop kontak di lokasi",
  },
  {
    index: "03",
    slug: "desain",
    match: ["DESAIN"],
    alt: "Desainer SAVOY menyusun visual 3D kitchen set di studio",
  },
  {
    index: "04",
    slug: "penawaran-rab",
  ...
  },
  {
    ...
    slug: "produksi",
    match: ["PRODUKSI"],
    alt: "Pengrajin memotong panel di workshop SAVOY",
  },
  {
    index: "06",
...
async function buildLogos() {
  ...
  const source = path.join(LOGO_SOURCE, "savoyfurniture.png");
  ...
  await writeFile(path.join(OUT_LOGO, "savoy-wordmark-light.png"), trimmed);
  ...
  const dark = await sharp({
    ...
    .png()
    .toBuffer();
  await writeFile(path.join(OUT_LOGO, "savoy-wordmark-dark.png"), dark);
